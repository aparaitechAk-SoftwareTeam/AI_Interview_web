...
export function LandingPage() {
  ...
  useEffect(()=>{
    ...
    return()=>{ window.removeEventListener("scroll",onScroll); window.removeEventListener("mousemove",onMove); window.removeEventListener("resize",resize); tiltElement?.removeEventListener("mousemove",onTilt); tiltElement?.removeEventListener("mouseleave",resetTilt); cancelAnimationFrame(frame); context.revert(); };
  },[]);
  ...
  return <div className="reference-home site-shell" ref={root}>{intro&&<Intro onDone={completeIntro}/>}<div className="noise" aria-hidden="true"/><div className="reference-cursor-glow" ref={glowRef} aria-hidden="true"/><canvas className="reference-particles" ref={canvasRef} aria-hidden="true"/>
    <header className={`nav-shell ${scrolled?"scrolled":""}`}><nav className={`nav container ${menu?"open":""}`}><a className="brand" href="#top" aria-label="Aparaitech AI Interview home"><Brand/></a><button className="menu-toggle" aria-label="Open navigation" aria-expanded={menu} onClick={()=>setMenu(!menu)}><span/><span/></button><div className="nav-links"><a href="#features" onClick={()=>setMenu(false)}>Features</a><a href="#how" onClick={()=>setMenu(false)}>How It Works</a><a href="#use-cases" onClick={()=>setMenu(false)}>Use Cases</a><a href="#resources" onClick={()=>setMenu(false)}>Resources</a></div><div className="nav-actions"><Link className="btn btn-ghost" href="/login">Login</Link><Link className="btn btn-primary small" href="/candidate/login">Get Started <span>↗</span></Link></div></nav></header>
